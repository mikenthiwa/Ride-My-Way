...
class AdminEndpoint(ConfigTestCase):
    ...
    def test_promote_user(self):
        ...
        res = self.client().patch('/api/v3/admin/users/chris_user@gmail.com', headers=self.admin_header)
        self.assertIn("Email is not available", str(res.data))

    def test_delete_user(self):
        """Test API can delete user"""

        response = self.client().delete('/api/v3/admin/users/test_user@gmail.com', headers=self.admin_header)
        self.assertEqual(response.status_code, 200)
        self.assertIn("user deleted", str(response.data))

        res = self.client().delete('/api/v3/admin/users/meme_user@gmail.com', headers=self.admin_header)
        self.assertIn("Email is not available", str(res.data))
