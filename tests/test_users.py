...
class UserEndpoint(ConfigTestCase):
    ...
    def test_change_username(self):
        ...
        res = self.client().put('/api/v1/auth/user/test_user@gmail.com', data = json.dumps(username), content_type='application/json')
        self.assertIn("username changed", str(res.data))

        # invalid user
        res1 = self.client().put('/api/v1/auth/user/chris@gmail.com', data = json.dumps(username), content_type='application/json')
        self.assertIn("Email is not available", str(res1.data))

    def test_reset_password(self):
        """Test API can change password"""
        password = {"password": "admin2018"}
        res = self.client().put('/api/v1/auth/user/test_user@gmail.com', data = json.dumps(password), content_type='application/json')
